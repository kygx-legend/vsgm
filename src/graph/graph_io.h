...
class GraphIO {
 public:
  static void ReadDataFile(const std::string& filename, bool directed, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols, GraphFormat graph_format) {
    switch (graph_format) {
      case SNAP:
        ReadSNAPFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      case BIN:
        ReadCSRBinFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      case SNAP_LVID:
        ReadSNAPLargeVIDFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      default:
        assert(false);
    }
  ...
  static void ReadDataFile(const std::string& filename, bool directed, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols) {
    ...
    ReadDataFile(filename, directed, vertex_count, edge_count, row_ptrs, cols, graph_format);
  }

  static void ReadCSRBinFile(const std::string& filename, bool directed, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols) {
    ...
    TimeMeasurer timer;
    timer.StartTimer();
    std::cout << "start read csr bin file...." << std::endl;
    ...
    std::cout << "uintV_size=" << uintV_size << ",uintE_size=" << uintE_size << std::endl;
    ...
    timer.EndTimer();
    std::cout << "finish read csr bin file, elapsed_time=" << timer.GetElapsedMicroSeconds() / 1000.0 << "ms" << std::endl;
  }
  ...
  static void ReadSNAPLargeVIDFile(const std::string& filename, bool directed, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols) {
    vertex_count = 0;
    edge_count = 0;
    row_ptrs = NULL;
    cols = NULL;
  ...
  static void ReadSNAPFile(const std::string& filename, bool directed, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols) {
    vertex_count = 0;
    edge_count = 0;
    row_ptrs = NULL;
    cols = NULL;
  ...
  static void ReadFromVector(std::vector<std::vector<uintV>>& data, size_t& vertex_count, size_t& edge_count, uintE*& row_ptrs, uintV*& cols) {
  ...
  static void WriteDataFile(GraphFormat graph_format, const std::string& filename, bool directed, size_t vertex_count, size_t edge_count, uintE* row_ptrs, uintV* cols) {
    switch (graph_format) {
      case SNAP:
        WriteCSRSNAPFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      case BIN:
        WriteCSRBinFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      case ADJ:
        WriteCSRAdjFile(filename, directed, vertex_count, edge_count, row_ptrs, cols);
        break;
      default:
        assert(false);
    }
  ...
  static void WriteCSRBinFile(const std::string& filename, bool directed, size_t vertex_count, size_t edge_count, uintE* row_ptrs, uintV* cols) {
    TimeMeasurer timer;
    timer.StartTimer();
    ...
    fclose(file_out);
    timer.EndTimer();
    std::cout << "finish write csr bin file, output_filename=" << output_filename << ",elapsed_time=" << timer.GetElapsedMicroSeconds() / 1000.0 << "ms" << std::endl;
  }

  static void WriteCSRSNAPFile(const std::string& filename, bool directed, size_t vertex_count, size_t edge_count, uintE* row_ptrs, uintV* cols) {
    TimeMeasurer timer;
    timer.StartTimer();
    std::cout << "start write csr snap file...." << std::endl;
    ...
    file.close();
    timer.EndTimer();
    std::cout << "finish write csr snap file, output_filename=" << output_filename << ",elapsed_time=" << timer.GetElapsedMicroSeconds() / 1000.0 << "ms" << std::endl;
  }

  // for fennel
  static void WriteCSRAdjFile(const std::string& filename, bool directed, size_t vertex_count, size_t edge_count, uintE* row_ptrs, uintV* cols) {
    TimeMeasurer timer;
    timer.StartTimer();
    std::cout << "start write csr adj file...." << std::endl;
    ...
    timer.EndTimer();
    std::cout << "finish write csr adj file, output_filename=" << output_filename << ",elapsed_time=" << timer.GetElapsedMicroSeconds() / 1000.0 << "ms" << std::endl;
  }
...
#pragma omp parallel for schedule(dynamic)
  ...
  static void SortCSRArray(size_t vertex_count, uintE* row_ptrs, uintV* cols, size_t thread_num) {
