...
#include "gpu/gpu_utils_filter.h"
#include "query/dev_plan.h"

// For a set of connectivity as in 'conn', find the level that has the
// smallest adjacent list.
DEVICE uintV ThreadChoosePivotLevel(DevConnType& conn, uintV* M, uintE* row_ptrs_data) {
  uintV pivot_level = 0;
...
DEVICE bool ThreadCheckConnectivity(DevConnType& conn, uintV* M, uintV candidate, char pivot_level, uintE* row_ptrs_data, uintV* cols_data) {
  ...
  for (size_t i = 0; i < check_conn.GetCount(); ++i) {
    ...
    size_t find_pos = GpuUtils::BinSearch::BinSearch(search_array, search_array_size, candidate);
    if (!(find_pos < search_array_size && search_array[find_pos] == candidate)) {
      return false;
    }
  }
  return true;
...
DEVICE bool ThreadCheckCondition(DevCondArrayType& cond, uintV* M, uintV candidate) {
  for (size_t i = 0; i < cond.GetCount(); ++i) {
    DevCondType c = cond.GetCondition(i);
    if (!GpuUtils::Filter::CheckVertexCondition(candidate, M[c.GetOperand()], c.GetOperator())) {
      return false;
    }
  }
  return true;
...
DEVICE bool ThreadCheckDuplicate(DevConnType& conn, uintV* M, uintV candidate, uintP* d_partition_ids) {
  for (size_t i = 0; i < conn.GetCount(); ++i) {
    uintV u = conn.Get(i);
    uintV v = M[u];
...
DEVICE bool ThreadCheckDuplicate(DevConnType& conn, uintV* M, uintV candidate, uintP* d_partition_ids, uintV prime_edge_v0, uintV prime_edge_v1) {
  for (size_t i = 0; i < conn.GetCount(); ++i) {
    uintV u = conn.Get(i);
    uintV v = M[u];
    if (d_partition_ids[candidate] != d_partition_ids[v]) {
#if defined(DEBUG)
      assert(d_partition_ids[M[prime_edge_v0]] != d_partition_ids[M[prime_edge_v1]]);
#endif
      bool f = ThreadCheckDuplicate(M[prime_edge_v0], M[prime_edge_v1], v, candidate);
      if (!f) {
        return false;
      }
    }
...
DEVICE size_t ThreadCountWithComputedVertex(uintV* search_array, size_t search_count, uintV search_element, CondOperator cond_operator) {
  if (search_count == 0) {
    return 0;
  }
...
DEVICE size_t ThreadEstimatePathCount(DevConnType& conn, uintV* M, uintE* row_ptrs) {
  size_t ret;
  if (op == ADD) {
    ret = 0;
  } else if (op == MAX) {
