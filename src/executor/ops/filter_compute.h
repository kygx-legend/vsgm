...
#include "gpu/gpu_utils_iterator.h"
#include "gpu/gpu_utils_segment_reduce.h"
...
static void FilterCandidate(
    WorkContext* wctx, uintV filter_level, PathIdIterator path_iterator, size_t path_num, DevCondArrayType* cond, DeviceArray<size_t>*& new_candidates_offsets, DeviceArray<uintV>*& new_candidates) {
  ...
  size_t* cur_candidates_offsets = im_data->GetCandidatesOffsets()[filter_level]->GetArray();
  size_t* cur_candidates_indices = im_data->GetCandidatesIndices()[filter_level]->GetArray();

  auto d_seq_instances = im_data_holder->GetSeqInstances()->GetArray();

  DeviceArray<size_t>* path_offsets = new DeviceArray<size_t>(path_num + 1, context);
  GpuUtils::Scan::TransformScan(
      [=] DEVICE(int index) {
        size_t path_id = path_iterator[index];
        size_t pos = cur_candidates_indices[path_id];
        size_t count = cur_candidates_offsets[pos + 1] - cur_candidates_offsets[pos];
        return count;
      },
  ...
  if (total_children_count == 0) {
    ...
    GpuUtils::Compact::Compact(children, total_children_count, bitmaps->GetArray(), new_candidates, compact_output_count, context);
    delete children;
    children = NULL;
    // CAUTION: it is possible that compact_output_count = 0

    DeviceArray<size_t>* new_candidates_count = new DeviceArray<size_t>(path_num, context);
    GpuUtils::SegReduce::TransformSegReduce(
        [=] DEVICE(int index) { return bitmaps_data[index] ? 1 : 0; }, total_children_count, path_offsets->GetArray(), path_num, new_candidates_count->GetArray(), (size_t)0, context);
    delete bitmaps;
    bitmaps = NULL;

    new_candidates_offsets = new DeviceArray<size_t>(path_num + 1, context);
    GpuUtils::Scan::ExclusiveSum(new_candidates_count->GetArray(), path_num, new_candidates_offsets->GetArray(), new_candidates_offsets->GetArray() + path_num, context);
...
static void FilterCompute(WorkContext* wctx, uintV filter_level) {
  auto context = wctx->context;
  auto im_data = wctx->im_data;
  auto im_data_holder = wctx->im_data_holder;
  ...
  FilterCandidate(wctx, filter_level, GpuUtils::Iterator::CountingIterator<size_t>(0), path_num, filter_order, new_candidates_offsets, new_candidates);
  ...
  GpuUtils::Transform::Sequence(d_candidates_indices[filter_level]->GetArray(), path_num, (size_t)0, context);
}
