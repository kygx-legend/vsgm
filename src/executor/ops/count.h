...
static void GeneralCountTwoComputedVertices(
    uintV u0,
    uintV u1,
    const CondOperator cond_operator,
    size_t path_num,
    size_t* candidates_indices0_data,
    size_t* candidates_indices1_data,
    size_t* candidates_offsets0_data,
    size_t* candidates_offsets1_data,
    uintV* candidates0_data,
    uintV* candidates1_data,
    size_t* d_total_count_data,
    CudaContext* context) {
  ...
  GpuUtils::Scan::TransformScan(
      [=] DEVICE(int index) {
        ...
        size_t count0 = candidates_offsets0_data[p0 + 1] - candidates_offsets0_data[p0];
        size_t p1 = from_index ? candidates_indices1_data[index] : index;
        size_t count1 = candidates_offsets1_data[p1 + 1] - candidates_offsets1_data[p1];
        size_t ret;
        if (count0 <= count1) {
          ret = count0;
        } else {
      ...
      path_num, workload_offsets.GetArray(), workload_offsets.GetArray() + path_num, context);
  ...
  GpuUtils::LoadBalance::LoadBalanceSearch<MGPULaunchBoxVT1>(total_workload_count, workload_offsets.GetArray(), path_num, path_ids.GetArray(), context);
  ...
  auto k = [=] DEVICE(int index) {
    ...
    size_t bin_pos = GpuUtils::BinSearch::BinSearch(search_array, search_count, search_element);
    bool equal = (bin_pos < search_count && search_array[bin_pos] == search_element);
  ...
  GpuUtils::Reduce::TransformReduce(k, total_workload_count, d_total_count_data, context);
}

static size_t LIGHTCount(WorkContext* wctx, size_t cur_exec_level) {
  ...
  auto d_seq_candidates_indices = im_data_holder->GetSeqCandidatesIndices()->GetArray();
  auto d_seq_candidates_offsets = im_data_holder->GetSeqCandidatesOffsets()->GetArray();

  // materialized_vertices and unmaterialized_vertices
  auto& materialized_vertices = plan->GetMaterializedVertices()[cur_exec_level];
  auto& unmaterialized_vertices = plan->GetComputedUnmaterializedVertices()[cur_exec_level];

  auto d_materialized_vertices = dev_plan->GetMaterializedVertices()->GetArray() + cur_exec_level;
  auto d_unmaterialized_vertices = dev_plan->GetComputedUnmaterializedVertices()->GetArray() + cur_exec_level;
  ...
  if (unmaterialized_vertices.size() == 1) {
    ...
    size_t* cur_candidates_indices = d_candidates_indices[cur_level]->GetArray();
    size_t* cur_candidates_offsets = d_candidates_offsets[cur_level]->GetArray();
    auto k = [=] DEVICE(int index) {
      size_t p = cur_candidates_indices[index];
      size_t count = cur_candidates_offsets[p + 1] - cur_candidates_offsets[p];
      return count;
    };
    GpuUtils::Reduce::TransformReduce(k, path_num, total_count.GetArray(), context);

  } else if (unmaterialized_vertices.size() == 2) {
    uintV u0 = unmaterialized_vertices[0];
    uintV u1 = unmaterialized_vertices[1];
    CondOperator cond_operator = GetConditionType(u0, u1, plan->GetQuery()->GetOrder());
    CUDA_ERROR(cudaMemsetAsync(total_count.GetArray(), 0, sizeof(size_t), context->Stream()));
    if (d_candidates[u0]->GetSize() > 0 && d_candidates[u1]->GetSize() > 0) {
      GeneralCountTwoComputedVertices<true>(
          u0, u1, cond_operator, path_num, d_candidates_indices[u0]->GetArray(), d_candidates_indices[u1]->GetArray(), d_candidates_offsets[u0]->GetArray(), d_candidates_offsets[u1]->GetArray(),
          d_candidates[u0]->GetArray(), d_candidates[u1]->GetArray(), total_count.GetArray(), context);
    }
