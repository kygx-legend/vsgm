#include "preprocess/preprocess_graph.h"
...
int main(int argc, char* argv[]) {
  if (argc == 1) {
    std::cout << "./preprocess_graph -f GRAPH_FILENAME(SNAP format) -d "
              ...
              << std::endl;
    std::cout << "OUTPUT_FORMAT: " << GraphFormat::SNAP << "  for .snap, " << GraphFormat::BIN << " (Default) for .bin, " << GraphFormat::ADJ << " for .adj, " << std::endl;
    return -1;
  }
  ...
  GraphIO::Validate(graph->GetVertexCount(), graph->GetRowPtrs(), graph->GetCols());
  GraphIO::WriteDataFile((GraphFormat)output_format, filename, directed, graph->GetVertexCount(), graph->GetEdgeCount(), graph->GetRowPtrs(), graph->GetCols());
