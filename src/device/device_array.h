...
class DeviceArray {
 public:
  ...
  DeviceArray(size_t size, CudaContext* context) : size_(size), context_(context) {
    alloc_ = true;
  ...
  // deleted
  DeviceArray(T* array, size_t size, CudaContext* context, bool alloc) : array_(array), size_(size), context_(context), alloc_(alloc) {}
  ...
  // ============ setter ==============
  HOST_DEVICE void SetSize(size_t size) { size_ = size; }
  ...
  void Print() const {
    ...
    CUDA_ERROR(cudaMemcpy(h_array, array_, sizeof(T) * size_, cudaMemcpyDeviceToHost));
...
template <typename T>
void BuildTwoDimensionDeviceArray(DeviceArray<T*>*& to, const std::vector<DeviceArray<T>*>* from, CudaContext* context) {
  ...
  CUDA_ERROR(cudaMemcpy(to->GetArray(), h_arrays.data(), sizeof(T*) * dimension, cudaMemcpyHostToDevice));
}

template <typename T>
void CopyTwoDimensionDeviceArray(DeviceArray<T*>* to, const std::vector<DeviceArray<T>*>& from) {
  size_t n = from.size();
  T** d_ptrs = new T*[n];
  for (size_t i = 0; i < n; ++i) {
    d_ptrs[i] = from[i] ? from[i]->GetArray() : NULL;
  }
  CUDA_ERROR(cudaMemcpy(to->GetArray(), d_ptrs, sizeof(T*) * n, cudaMemcpyHostToDevice));
  delete[] d_ptrs;
  d_ptrs = NULL;
}

template <typename T>
void AsyncCopyTwoDimensionDeviceArray(DeviceArray<T*>* to, const std::vector<DeviceArray<T>*>& from, cudaStream_t stream) {
  size_t n = from.size();
  T** d_ptrs = new T*[n];
  for (size_t i = 0; i < n; ++i) {
    d_ptrs[i] = from[i] ? from[i]->GetArray() : NULL;
  }
  CUDA_ERROR(cudaMemcpyAsync(to->GetArray(), d_ptrs, sizeof(T*) * n, cudaMemcpyHostToDevice, stream));
  delete[] d_ptrs;
  d_ptrs = NULL;
}
